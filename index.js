...
class AllureReporter {
    ...
    item(err, args) {
        ...
        const reqBodyTableStr =  createDataTableHTML('Request Body', bodyModePropObj);
        const reqHeadersTableStr = createDataTableHTML('Request Headers', rItem.pm_item.request_data.headers);
        ...
        const resBodyTable = createDataTableHTML('Response Body', responseBodyFormatted);
        const resHeaderTable = createDataTableHTML('Response Header', rItem.pm_item.response_data.headers);

        this.setDescriptionHtml(`
            <p>${testDescription}</p>
            <p><h4> Request: </h4></p>
            <p style="color: #888888">${requestDataURL}</p>
            <p>${reqHeadersTableStr}</p>
            <p>${reqBodyTableStr}</p>
            <h4> Response: </h4>
